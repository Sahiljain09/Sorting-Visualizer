...
async function pauseAnimation() {
  while (run == false) await sleep(10);
  return;
}
function sleep(ms) {
...
bubblesort.addEventListener("click", async () => {
  ...
  for (let i = 0; i < len; i++) {
    for (let j = 0; j < len - i - 1; j++) {
      ...
      if (run === false) await pauseAnimation();
      child[j].style.backgroundColor = "#12c012";
      child[j + 1].style.backgroundColor = "#12c012";
    }
  }
...
selectionsort.addEventListener("click", async () => {
  ...
  for (let i = 0; i < len; i++) {
    ...
    for (let j = i + 1; j < len; j++) {
      ...
      await sleep(10);
      if (h1 > h2) {
        child[min_index].style.backgroundColor = "#12c012";
        min_index = j;
        child[j].style.backgroundColor = "yellow";
      } else {
        child[j].style.backgroundColor = "#12c012";
    ...
    child[min_index].style.backgroundColor = "#12c012";
    child[i].style.backgroundColor = "#12c012";
  }
  slider.disabled = false;
  quicksort1.disabled = false;
  quicksort2.disabled = false;
...
async function quickSort(l, r) {
  if (l <= r) {
    ...
    child[pivot].style.backgroundColor = "#12c012";
...
async function heapSort() {
  ...
  child[0].style.backgroundColor = "#12c012";
}